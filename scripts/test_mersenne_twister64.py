#!/usr/bin/env python3
"""Check the caller-owned MT19937-64 ABI against a platform-independent algorithm reference."""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="dynlex-mt64-native-") as temporary:
        ...
        llvm_output = output / "mersenne_twister64_native.ll"
        run(
            [
                str(compiler),
        ...
        names = abi_symbols(llvm_output.read_text(encoding="utf-8"))
        required = {
        ...
        selected = {key: one_symbol(names, prefix) for key, prefix in required.items()}
...
extern "C" void seed_fn(void*, std::uint64_t) __asm__(DYNLEX_SYMBOL("{selected['seed']}"));
extern "C" std::uint64_t next_fn(void*) __asm__(DYNLEX_SYMBOL("{selected['next']}"));
extern "C" double fraction64_fn(void*) __asm__(DYNLEX_SYMBOL("{selected['fraction64']}"));
extern "C" float fraction32_fn(void*) __asm__(DYNLEX_SYMBOL("{selected['fraction32']}"));
...
static int checkEndpoint(std::uint64_t output) {{
    ...
    const float expected32 = random_reference::unit_fraction<float>(expected);
    ...
    const double expected64 = random_reference::unit_fraction<double>(expected);
...
int main() {{
    ...
    for (std::uint64_t seed : seeds) {{
        ...
        for (int draw = 0; draw < 10000; ++draw) {{
            ...
            const float expected32 = random_reference::unit_fraction<float>(expectedFractions);
            ...
            const double expected64 = random_reference::unit_fraction<double>(expectedFractions);
...
''',
        ...
        for optimization in ("-O0", "-O2", "-O3"):
            ...
            run([cxx, "-std=c++20", "-I", str(root / "tests/native"), "-O2", str(source), str(optimized_object), "-o", str(executable)], root)
