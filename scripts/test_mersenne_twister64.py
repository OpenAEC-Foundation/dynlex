...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="dynlex-mt64-native-") as temporary:
        ...
        object_file = output / "mersenne_twister64_native.o"
        run(
            [
                str(compiler),
...
int main() {{
    ...
    for (std::uint64_t seed : seeds) {{
        ...
        seed_fn(&second, seed);
        if (next_fn(&second) != resetExpected()) return 5;
    }}
    return 0;
}}
''',
        ...
        executable = output / "corpus"
        run([cxx, "-std=c++20", "-O2", str(source), str(object_file), "-o", str(executable)], root)
        result = subprocess.run([str(executable)], cwd=root, check=False)
        if result.returncode != 0:
            fail(f"native MT19937-64 corpus exited {result.returncode}")
    print("MT19937-64 native ABI corpus passed")
    return 0
