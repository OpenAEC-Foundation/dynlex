...
| `src/brep/sweep.rs` | `cadkernel::brep::sweep` | 3802 | 4602 | 35 | in-progress | `brep_sweep.dl`, `brep_region_solid.dl`, `brep_append.dl`, `brep_profile_pieces.dl`, `brep_sweep_polyline3d.dl`, `brep_sweep_planar.dl`, `brep_revolve_region.dl`, `brep_revolve_sheet.dl`; closed/open extrusion, capped partial/full revolution, tapered polygon/circle solids and closed/open tapered sheets pass native O0/O2 fixtures; circular draft retains an analytic cone while oblique segmented circles use ruled surfaces; three open-taper cases match 674 pinned-source scalar comparisons; 46 lateral/solid/tapered-region cases match 8,416 fields with exact native O0/O2 parity; nine 3D-polyline path cases match 632 fields, eleven ordinary planar path cases match 406 fields, fifteen solid revolved-region cases match 1,438 fields and 35 revolved-sheet cases match 1,722 fields, all at O0/O2; append-body remaps all nine arenas through indexed dense/sparse source-slot tables, including 512-sheet and high-slot sparse-arena stress cases; mitered rectangular/deformed path operations, ellipse/NURBS revolution profiles, remaining source groups and full-module differential comparison pending |
