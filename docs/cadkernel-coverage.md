...
| `src/brep/sweep.rs` | `cadkernel::brep::sweep` | 3802 | 4602 | 35 | in-progress | `brep_sweep.dl`, `brep_region_solid.dl`, `brep_append.dl`, `brep_profile_pieces.dl`; closed/open extrusion, revolution, tapered polygon/circle solids and closed/open tapered sheets pass native O0/O2 fixtures; circular draft retains an analytic cone while oblique segmented circles use ruled surfaces; three open-taper cases match 674 pinned-source scalar comparisons; 34 lateral/solid region cases match 6,078 fields with exact native O0/O2 parity; append-body remaps all nine arenas through indexed dense/sparse source-slot tables, including 512-sheet and high-slot sparse-arena stress cases; tapered solid-region, revolution-region, path operations, remaining source groups and full-module differential comparison pending |
