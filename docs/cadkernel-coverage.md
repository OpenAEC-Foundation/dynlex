...
| `src/brep/sweep.rs` | `cadkernel::brep::sweep` | 3802 | 4602 | 35 | in-progress | `brep_sweep.dl`, `brep_append.dl`, `brep_profile_pieces.dl`; closed/open extrusion, revolution, tapered polygon/circle solids and closed/open tapered sheets pass native O0/O2 fixtures; circular draft retains an analytic cone; three open-taper cases match 674 pinned-source scalar comparisons with exact native O0/O2 parity; append-body remaps all nine arenas through indexed source-slot tables, including a 512-sheet linked-topology stress case; lateral region sheets preserve separate loops with O0/O2 topology checks; solid-region/path operations, remaining source groups and full-module differential comparison pending |
