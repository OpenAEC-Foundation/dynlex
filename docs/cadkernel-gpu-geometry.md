...
from the DynLex CAD kernel. This establishes planar surface evaluation on the
GPU, but it is not yet connected to the B-rep mesher. The probe's SPIR-V is
...
rejected shader-stage operations; and run the dispatch and plane checks at O0
and O2. The test needs a built `build/dynlex.exe` and Vulkan runtime. Full
