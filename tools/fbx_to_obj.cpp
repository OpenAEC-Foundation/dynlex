...
void print_usage(const char *argv0) { std::cerr << "Usage: " << argv0 << " <input.fbx> <output.obj> [--triangle-step N]\n"; }
...
bool load_texture_rgb(
	const ufbx_texture *texture, const fs::path &resolved_source, int &width, int &height, std::vector<uint8_t> &rgb
) {
	width = 0;
	height = 0;
	rgb.clear();
	...
	if (!pixels && texture) {
		for (size_t i = 0; i < texture->file_textures.count; ++i) {
			...
			if (file_texture->content.size > 0 && file_texture->content.data) {
				pixels = stbi_load_from_memory(
					reinterpret_cast<const stbi_uc *>(file_texture->content.data), static_cast<int>(file_texture->content.size),
...
fs::path pick_texture_source_path(
	...
	auto find_texture_by_stem = [&](const fs::path &requested_name) -> std::optional<fs::path> {
		...
		for (const fs::directory_entry &entry :
			 fs::recursive_directory_iterator(texture_root, fs::directory_options::skip_permission_denied, ec)) {
			if (ec) {
				continue;
			}
