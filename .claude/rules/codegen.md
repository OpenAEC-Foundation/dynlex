...
- `currentInstantiation` pointer on `InferenceContext`: set during non-flex function body inference (not on ParseContext)
...
## Scoped Flex Bindings
- `flexBindingStack` (`std::stack`) on ParseContext: pushed on flex entry, popped on exit
- `resolveVariableBinding` (codegenTypes.cpp): resolves one Variable through the current flex's binding map. Each resolution crosses one scope boundary — caller must pop before evaluating the result
- `resolveThroughFlexLayers` (codegenTypes.cpp): resolves fully through variable bindings AND flex PatternCall expansions, crossing scope boundaries as needed (pops to parent scopes when a variable isn't found in the current scope). Freely modifies scope state — callers must save/restore `flexExpressionBindings` + `flexBindingStack`. Use when inspecting the underlying expression kind (e.g., detecting property intrinsic in store dest)
- `expandFlexPatternCall` (parseContext.h): extracts flex body + parameter bindings from a PatternCall. Shared by codegen and type inference. Does not modify any binding stack
...
- `resolveThroughBindingsDeep` (typeInference.cpp): type inference counterpart of `resolveThroughFlexLayers` — resolves through variable bindings AND flex PatternCalls with explicit bindings. Returns the active bindings via output parameter
- `FlexScopeGuard` RAII: temporarily pops to caller scope when generating resolved argument expressions
- `getVariablePointer`: iterates `resolveVariableBinding` + scope pops to resolve through nested flex bindings to find the actual variable's alloca
...
- `TRY_EXPR(var, expr)` flex in `codegenIntrinsics.cpp`: declares `llvm::Value *var`, calls `generateExpressionCode`, returns `false` from the enclosing function on failure
...
- **Flex binding variable capture**: `return value + 1000` caused infinite recursion. Fix: temporarily erase binding while generating resolved expression.
- **Flex bindings leaking into functions**: `generateSpecializedFunction` now saves/clears/restores `flexExpressionBindings`.
- **Caller flex bindings must never leak into monomorphized function bodies**: `generateSpecializedFunction` must isolate `flexBindingFrames` to a fresh root frame while emitting the callee body, then restore caller frames after emission. Pushing an empty child frame is insufficient because cross-frame lookup can still capture parent caller bindings by name.
- **Section flex locals must be allocated per invocation and restored after expansion**: section-flex replacement bodies can declare local variables (e.g., `set __i to 0`). Flex expansion must allocate replacement-section locals before emitting replacement lines, and restore the flex definition's `VariableReference::alloca` pointers after the call so nested/re-entrant flex calls do not overwrite each other's active local storage bindings.
- **Section flex body placement is explicit-capable and wrapper-safe**: section flexes still auto-emit the opened body after replacement by default, but `@intrinsic("execute body")` can emit that body at a specific point inside replacement code. Ownership resolution is deterministic and layered: source-section ancestry first, then active flex call-site section stack, then active flex-definition stack. This allows helper flex wrappers (for example `execute the body`) and nested loop-pattern flexes to resolve the intended caller body without name hacks.
- **Explicit body emission must not finalize loop control flow early**: when `execute body` is emitted explicitly inside a section flex, codegen must continue in the current replacement flow so lines after it (for example `increment index`) remain inside the loop body. Back-edge/exit finalization happens once at section-flex exit (or during fallback auto-emission) to avoid both infinite loops and duplicate body runs.
- **Cross-frame binding resolution must be single-step**: resolving a bound variable name across flex frames should return one substitution step at a time. Multi-step resolution in one pass can self-capture sibling flex parameters with the same names (e.g., `set var to val` with argument `val`) and silently drop stores.
- **Nested flex store**: `add value to target` → `set var to val` needed multi-level resolution. Fix: `getVariablePointer` recursively pops flex scopes. For property stores through nested flexes (e.g., `add value to the x of target`), `resolveThroughFlexLayers` now also crosses scope boundaries. The store handler generates the value first, then saves/restores scopes around dest resolution.
- **Stale flex body types**: Shared expression nodes retained types from prior callers. Fix: `resetSectionTypes()` before each `inferFlexBody` (flex sections only).
- **Flex-expanded `construct` nodes cannot rely on `expr->type` during codegen**: `cloneFlexExpansionExpression()` clears cloned expression types, so wrappers like `new string from data ...` reach codegen as fresh `@intrinsic("construct", ...)` nodes with `expr->type == unresolved`. `getEffectiveType()` must reconstruct the `construct` result type from its compile-time type argument instead of assuming inference annotations survived cloning.
- **Spurious instantiations**: Top-level inference created instantiations with undeduced args. Fix: skip non-flex function body inference when any arg is undeduced.
