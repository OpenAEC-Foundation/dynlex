...
macro real-inference, section-header outermost grouping, the matcher memory blow-up,
...
- `src/compiler/type_inference/type_resolution.inl` still does not resolve single-token `Pending` expressions through active bindings during compile-time type resolution. This breaks macro type parameters such as `elementtype`.
...
- Macro replacement bodies used to be inferred through the rollback-only probe path even during real inference in `src/compiler/type_inference/function_inference.inl`.
...
- Function-macro codegen used to re-expand raw replacement source instead of cloning the grouped macro body selected during type inference, so wrappers like `add 2 to total` could still reach codegen as `+ [void, i32]` even after inference had found the right grouping.
...
- `src/compiler/codegen/codegenTypes.cpp` used to assume macro-expanded `@intrinsic("construct", ...)` clones still carried `expr->type`.
