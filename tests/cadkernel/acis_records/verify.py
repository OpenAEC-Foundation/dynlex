...
"""Compare native SAT record behavior with the pinned in-memory codec model."""
...
def main() -> None:
    ...
    parent = ROOT / "build/cadkernel-acis-records-checks"
    ...
    (project / "Cargo.toml").write_text(
        '[package]\nname = "cadkernel_acis_records_reference"\nversion = "0.1.0"\nedition = "2021"\n'
        f'\n[dependencies]\nacadrust = {{ path = "{codec.as_posix()}" }}\n',
        encoding="utf-8",
    )
    ...
    reference = target / "debug/cadkernel_acis_records_reference.exe"
    if not reference.is_file():
        reference = reference.with_suffix("")
    expected = normalize_output(checked([reference], cwd=ROOT))
    fixture = ROOT / "tests/required/cadkernel_acis_records"
    compare("pinned codec vs fixture oracle", normalize_output((fixture / "expected.txt").read_text(encoding="utf-8")), expected)

    for mode in ("O0", "O2"):
        result = verify_fixture(fixture, mode, compiler, artifacts, 120, 20, False)
        actual = normalize_output(checked([artifacts / f"cadkernel_acis_records-{mode}.out"], cwd=ROOT))
        compare(f"pinned codec vs native {mode}", expected, actual)
        print(f"{mode}: {result}; {len(expected.splitlines())} differential cases match", flush=True)
    digest = hashlib.sha256(compiler.read_bytes()).hexdigest()
    print(f"Source pins: {SOURCE_PIN}, {CODEC_PIN}")
    print(f"Compiler SHA256: {digest}")
