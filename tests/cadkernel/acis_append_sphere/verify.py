...
"""Compare in-memory sphere SAT append with the pinned kernel and codec."""
...
def main() -> None:
    ...
    parent = ROOT / "build/cadkernel-acis-append-sphere-checks"
    ...
    (project / "Cargo.toml").write_text(
        '[package]\nname = "cadkernel_acis_append_sphere_reference"\nversion = "0.1.0"\nedition = "2021"\n'
    ...
    reference = target / "debug/cadkernel_acis_append_sphere_reference.exe"
    ...
    fixture = ROOT / "tests/required/cadkernel_acis_append_sphere"
    ...
    for mode in ("O0", "O2"):
        ...
        binary = artifacts / f"cadkernel_acis_append_sphere-{mode}.out"
        actual = normalize_output(checked([binary], cwd=ROOT))
        compare(f"pinned source vs native {mode}", expected, actual)
        print(f"{mode}: {result}; {len(expected.splitlines())} complete record lines match", flush=True)
