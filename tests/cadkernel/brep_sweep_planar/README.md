...
The current cases use a closed triangular section, so they exercise the ordinary segment-and-union path. Rectangular line-path mitering runs through a separate native module and fixture, called first by the same public planar sweep. Deformation controls remain outside this operation.
