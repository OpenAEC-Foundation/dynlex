...
"""Compare the native Box route with pinned public Rust history at O0/O2."""
...
def cases():
    yield "identity", 1, (2., 3., 4.), IDENTITY, 0
    yield "turn", 1, (2., 3., 4.), TURN, 0
    yield "uniform-scale", 1, (2., 3., 4.), SCALE, 0
    yield "reflection", 1, (2., 3., 4.), REFLECT, 0
    yield "unit", 1, (1., 1., 1.), IDENTITY, 0
    yield "small", 1, (.125, .25, .5), TURN, 0
    yield "fractional", 1, (1.25, 2.5, 3.75), REFLECT, 0
    yield "small-affine-residue", 1, (2., 3., 4.), matrix(column=(5e-10, -5e-10, 5e-10, 1.)), 0
    yield "translation", 1, (2., 3., 4.), matrix(origin=(512345.678, 4512345.678, 91.5)), 0
    ...
    for name, size in (("zero-length", (0., 3., 4.)), ("negative-width", (2., -3., 4.)),
        ...
        yield name, 1, size, IDENTITY, 2 if name != "subnormal-width" else None
    for name, bad in (("bad-column-3", matrix(column=(2e-9, 0., 0., 1.))),
        ...
        yield name, 1, (2., 3., 4.), bad, 1
    yield "invalid-dimensions-win", 1, (0., 3., 4.), SHEAR, 2
    for index in range(8):
        size = (1. + index / 8., 2. + index / 4., 3. + index / 2.)
        place = matrix((0., 1., 0.), (-1., 0., 0.), (0., 0., 1.),
                       (index - 3., 2. * index, -index / 2.))
        yield f"placed-{index}", 1, size, place, 0
...
def main() -> None:
    ...
    parent = ROOT / "build/cadkernel-acis-history-box-checks"
    ...
    (project / "Cargo.toml").write_text(
        '[package]\nname = "cadkernel_acis_history_box_reference"\nversion = "0.1.0"\nedition = "2021"\n'
    ...
    fixture = ROOT / "tests/required/cadkernel_acis_history_box"
    count = fields = 0
    for mode in ("O0", "O2"):
        ...
        reference = target / ("release" if mode == "O2" else "debug") / "cadkernel_acis_history_box_reference.exe"
