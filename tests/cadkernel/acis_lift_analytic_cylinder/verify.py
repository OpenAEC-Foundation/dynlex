...
"""Compare native cylinder SAT lift with pinned kernel and codec."""
...
def main() -> None:
    ...
    parent = ROOT / "build/cadkernel-acis-lift-cylinder-checks"
    ...
    (project / "Cargo.toml").write_text(
        '[package]\nname = "cadkernel_acis_lift_cylinder_reference"\nversion = "0.1.0"\nedition = "2021"\n'
    ...
    reference = target / "debug/cadkernel_acis_lift_cylinder_reference.exe"
    if not reference.is_file():
        reference = reference.with_suffix("")
    expected = normalize_output(checked([reference], cwd=ROOT))
    fixture = ROOT / "tests/required/cadkernel_acis_lift_analytic_cylinder"
    ...
    for mode in ("O0", "O2"):
        ...
        actual = normalize_output(checked([artifacts / f"cadkernel_acis_lift_analytic_cylinder-{mode}.out"], cwd=ROOT))
