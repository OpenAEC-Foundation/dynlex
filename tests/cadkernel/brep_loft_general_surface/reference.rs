...
use space::{NurbsSurface3, Plane, Vec3};
...
fn refusal(_code: f64) { emit(1.0); emit(0.0); emit(1.0); }
fn emit_point(point: [f64; 3]) { for coordinate in point { emit(coordinate); } }
fn main() {
    ...
    let regular = next(&args, &mut cursor) != 0.0;
    ...
    for column in 0..breaks.len() - 1 {
        ...
        let Ok(surface) = patch.surface() else { emit(0.0); emit(2.0); continue; };
        let normal = surface.normal_at(0.5, 0.5);
        if regular && normal.is_none() { emit(0.0); emit(3.0); continue; }
        emit(1.0); emit(0.0);
        emit(breaks[column]); emit(breaks[column + 1]);
        let (u_degree, v_degree) = surface.degrees();
        emit(u_degree as f64); emit(v_degree as f64);
        let periodicity = surface.periodicity();
        emit(periodicity[0] as u8 as f64); emit(periodicity[1] as u8 as f64);
        emit(surface.v_reversed() as u8 as f64);
        ...
        emit(surface.control_points().len() as f64);
        for (points, weights) in surface.control_points().iter().zip(surface.weights()) {
            emit(points.len() as f64);
            for (point, weight) in points.iter().zip(weights) { emit_point(*point); emit(*weight); }
        }
        emit(normal.is_some() as u8 as f64);
        if let Some(normal) = normal { emit_point(normal); }
        emit_point(surface.point_at(0.25, 0.75));
    }
}
