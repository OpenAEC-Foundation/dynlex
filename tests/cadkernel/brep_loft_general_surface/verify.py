...
"""Compare pinned Rust Patch::surface() and midpoint regularity with DynLex."""
...
HASHES = {
    ...
    "src/space/nurbs.rs": "d3f60efe2c55573009c6d141e44eb532eb0d53fe258864c260034ae2c0075458",
}
...
def driver_text(source: Path) -> str:
    ...
    )) + "\n}\nimpl Patch {\n" + "\n".join(source_function(text, signature, text.index("impl Patch {")) for signature in (
        "fn u_degree(&self)", "fn v_degree(&self)", "fn surface(&self)",
    )) + "\n}\n"
    functions = "\n\n".join(source_function(text, signature) for signature in (
        "fn mix(", "fn project(", "fn homogeneous(", "fn de_casteljau(", "fn bezier_knots(",
...
class Case:
    ...
    def arguments(self) -> list[str]:
        values = [int(self.cyclic), int(self.matching), self.mode, int(self.periodic),
                  ...
                  self.wire_index, self.band, int(self.regular), len(self.sections)]
...
def cases() -> list[Case]:
    ...
    result = [replace(item, error=(1 if item.error else
                                   (0, 0, 3, 0, 0, 0, 0) if item.name == "nonuniform-spline-seam" else
                                   (0, 3, 0, 3) if item.name == "nonuniform-spline-winding" else
                                   3 if item.name in ("disabled-alignment-preserves-seam", "open-chains-disabled") else 0))
              for item in result]
    point_wire = ((line((0., 0.), (0., 0.)),),)
    result.extend([
        Case("midpoint-degenerate-refusal", (
            Section(frame(0.), point_wire, closed=False),
            Section(frame(4.), point_wire, closed=False),
        ), error=3),
        Case("midpoint-degenerate-allowed-before-assembly", (
            Section(frame(0.), point_wire, closed=False),
            Section(frame(4.), point_wire, closed=False),
        ), regular=False),
    ])
...
def compare(name: str, rust: str, native: str, expected_error: int | tuple[int, ...]) -> int:
    ...
    def statuses(values: list[float]) -> list[tuple[int, int]]:
        position = 1
        found = []
        for _ in range(int(values[0])):
            valid, error = map(int, values[position:position + 2]); position += 2
            found.append((valid, error))
            if not valid:
                continue
            position += 7  # interval, degrees, periodicity and v reversal
            knot_count = int(values[position]); position += 1 + knot_count
            knot_count = int(values[position]); position += 1 + knot_count
            row_count = int(values[position]); position += 1
            for _ in range(row_count):
                control_count = int(values[position]); position += 1 + 4 * control_count
            normal = int(values[position]); position += 1 + 3 * normal
            position += 3  # sample point
        if position != len(values):
            raise AssertionError(f"{name}: malformed surface output")
        return found

    rust_status = statuses(left)
    native_status = statuses(right)
    if rust_status != native_status:
        raise AssertionError(f"{name}: Rust status {rust_status}, DynLex {native_status}")
    expected = (expected_error,) * len(rust_status) if isinstance(expected_error, int) else expected_error
    if tuple(code for _, code in rust_status) != expected or any(valid != (code == 0) for valid, code in rust_status):
        UNEXPECTED.append(f"{name}: source statuses {rust_status}, expected {expected}")
...
def main() -> None:
    ...
    tested = fields = 0
    with tempfile.TemporaryDirectory(prefix="cad-loft-general-surface-", dir=ROOT / "build") as directory:
        ...
        for mode, directory_name in (("O0", "debug"), ("O2", "release")):
            ...
            for item in cases():
                ...
                tested += 1
            if not args.case:
                fixture = ROOT / "tests/required/cadkernel_brep_loft_general_surface"
                if (fixture / "main.dl").is_file():
    ...
    if UNEXPECTED:
        raise AssertionError("source expectations differ:\n" + "\n".join(UNEXPECTED))
