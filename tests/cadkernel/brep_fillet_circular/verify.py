...
"""Compare native circular filleting with the pinned source implementation."""
...
HERE = ROOT / "tests/cadkernel/brep_fillet_circular"
...
def encoded(profile, selected=(), radius=0.25, *,
    ...
    return [*origin, *radial, *height, *pivot, *axis, angle, len(profile),
            *(value for piece in profile for value in piece),
            len(selected), *selected, radius, damage, damage_value]
...
def cases():
    ...
    for edge in (0, 1):
        for radius in (1.0e-6, 0.05, 0.5, 1.5, 2.999999999, 3.0, 20.0):
            yield f"cylinder-rim-{edge}-radius-{radius}", encoded(cylinder, (edge,), radius)
    yield "cylinder-empty-selection", encoded(cylinder)
    yield "cylinder-duplicate-selection", encoded(cylinder, (0, 0), 0.4)
    yield "cylinder-both-rims", encoded(cylinder, (0, 1), 0.4)
    yield "cylinder-seam-selection", encoded(cylinder, (2,), 0.4)
    yield "cylinder-negative-turn", encoded(cylinder, (0,), 0.4, angle=-TAU)

    for edge in range(4):
        for radius in (0.05, 0.4, 1.0, 1.999999999, 2.0, 5.0):
            yield f"annulus-rim-{edge}-radius-{radius}", encoded(annulus, (edge,), radius)
    yield "annulus-opposite-rims", encoded(annulus, (0, 2), 0.35)
    yield "annulus-adjacent-rims", encoded(annulus, (0, 1), 0.35)
    yield "annulus-seam-selection", encoded(annulus, (4,), 0.35)

    for radius in (0.05, 0.5, 2.0, 4.999999999, 5.0, 8.0):
        yield f"cone-rim-radius-{radius}", encoded(cone, (0,), radius)
    for edge in range(3):
        yield f"torus-d-rim-{edge}", encoded(torus_d, (edge,), 0.25)
    yield "sphere-is-unrecognized", encoded(sphere, (0,), 0.25)
    ...
    for index, (origin, radial, height) in enumerate(orientations):
        yield f"oriented-cylinder-{index}", encoded(
            cylinder, (1,), 0.3, origin=origin, radial=radial, height=height,
        )
        yield f"oriented-annulus-{index}", encoded(
            annulus, (2,), 0.3, origin=origin, radial=radial, height=height,
        )

    for damage in range(1, 6):
        yield f"damaged-cylinder-{damage}", encoded(
            cylinder, (0,), 0.3, damage=damage, damage_value=2.0,
        )

    rng = random.Random(0xF111E7)
    for index in range(36):
        outer = rng.uniform(0.2, 15.0)
        low = rng.uniform(-12.0, 5.0)
        height = rng.uniform(0.2, 12.0)
        maximum = min(outer, height) * 0.49
        radius = rng.uniform(1.0e-5, maximum)
        edge = rng.randrange(2)
        profile = ring([(0.0, low), (outer, low), (outer, low + height), (0.0, low + height)])
        yield f"random-cylinder-{index}", encoded(profile, (edge,), radius)
    for index in range(36):
        inner_low = rng.uniform(0.2, 5.0)
        inner_high = rng.uniform(0.2, 5.0)
        outer_low = inner_low + rng.uniform(0.2, 6.0)
        outer_high = inner_high + rng.uniform(0.2, 6.0)
        height = rng.uniform(0.3, 10.0)
        profile = ring([(inner_low, 0.0), (outer_low, 0.0), (outer_high, height), (inner_high, height)])
        edge = rng.randrange(4)
        radius = rng.uniform(1.0e-5, 0.15)
        yield f"random-coaxial-trapezoid-{index}", encoded(profile, (edge,), radius)
...
def main():
    ...
    source_file = source / "src/brep/fillet_circular.rs"
    ...
    if "#[test]" in source_file.read_text(encoding="utf-8"):
        raise AssertionError("unexpected source tests in private circular fillet module")

    output = args.output or Path(tempfile.mkdtemp(prefix="brep-fillet-circular-", dir=ROOT / "build"))
    ...
    shutil.copy2(source_file, output / "fillet_circular_source.rs")
    binaries = {}
    compile_times = {}
    ...
    summary = {
        ...
        "native_source_sha256": {
            ...
            for path in (
                ...
                ROOT / "lib/cadkernel/brep_fillet_circular.dl",
