...
"""Compare analytic geometry write-back with pinned kernel and codec records."""
...
def main() -> None:
    ...
    parent = ROOT / "build/cadkernel-acis-lower-analytic-checks"
    ...
    (project / "Cargo.toml").write_text(
        '[package]\nname = "cadkernel_acis_lower_analytic_reference"\nversion = "0.1.0"\nedition = "2021"\n'
    ...
    reference = target / "debug/cadkernel_acis_lower_analytic_reference.exe"
    if not reference.is_file():
        reference = reference.with_suffix("")
    expected = normalize_output(checked([reference], cwd=ROOT))
    fixture = ROOT / "tests/required/cadkernel_acis_lower_analytic"
    ...
    for mode in ("O0", "O2"):
        ...
        actual = normalize_output(checked([artifacts / f"cadkernel_acis_lower_analytic-{mode}.out"], cwd=ROOT))
