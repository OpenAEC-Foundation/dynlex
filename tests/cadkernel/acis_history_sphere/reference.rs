...
use acadrust::objects::{SolidHistoryNodeBase, SolidHistoryOperation, SolidHistorySphere};
...
fn main() {
    ...
    let matrix = std::array::from_fn(|index| argument_number(&args, index + 2));
    let operation = SolidHistoryOperation::Sphere(SolidHistorySphere {
