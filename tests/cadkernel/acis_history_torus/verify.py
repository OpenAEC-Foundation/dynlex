...
"""Compare the native Torus route with pinned public Rust history at O0/O2."""
...
def cases():
    for shape, radii in (("ring", (3., 1.)), ("thin-ring", (3., .25)),
                         ("small-ring", (1e-6, 2e-7)),
                         ("large-ring", (1e6, 1e5)),
                         ("horn", (2., 2.)), ("unit-horn", (1., 1.)),
                         ("spindle", (1., 2.)), ("shallow-spindle", (.5, 1.))):
        for placement_name, placement in (("identity", IDENTITY), ("turn", TURN),
                                          ("scale", SCALE), ("reflect", REFLECT)):
            yield f"{shape}-{placement_name}", radii, placement, 0
    yield "near-horn-ring", (1.0000000000000002, 1.), IDENTITY, 0
    yield "near-horn-spindle", (1., 1.0000000000000002), IDENTITY, 0
    yield "survey", (3., 1.), matrix(origin=(512345.678, 4512345.678, 91.5)), 0
    yield "affine-residue", (3., 1.), matrix(column=(5e-10, -5e-10, 5e-10, 1.)), 0
    for label, radii in (("zero-major", (0., 1.)),
                         ("negative-zero-major", (-0., 1.)),
                         ("negative-major", (-3., 1.)),
                         ("nan-major", (math.nan, 1.)),
                         ("infinite-major", (math.inf, 1.)),
                         ("zero-minor", (3., 0.)),
                         ("negative-minor", (3., -1.)),
                         ("nan-minor", (3., math.nan)),
                         ("infinite-minor", (3., math.inf))):
        yield label, radii, IDENTITY, 2
    for label, placement in (
        ...
        yield label, (3., 1.), placement, 1
    yield "invalid-major-wins", (0., 1.), SHEAR, 2
    yield "invalid-minor-wins", (3., 0.), NONUNIFORM, 2
    yield "nan-major-wins", (math.nan, 1.), matrix(column=(2e-9, 0., 0., 1.)), 2
    yield "nan-minor-wins", (3., math.nan), ZERO, 2
    for index in range(8):
        placement = matrix((0., 1., 0.), (-1., 0., 0.), (0., 0., 1.),
                           (index - 3., 2. * index, -index / 2.))
        radii = (2. + index / 8., .5 + index / 16.)
        yield f"placed-{index}", radii, placement, 0
...
def main() -> None:
    ...
    parent = ROOT / "build/cadkernel-acis-history-torus-checks"
    ...
    (project / "Cargo.toml").write_text(
        '[package]\nname = "cadkernel_acis_history_torus_reference"\nversion = "0.1.0"\nedition = "2021"\n'
    ...
    fixture = ROOT / "tests/required/cadkernel_acis_history_torus"
    count = fields = 0
    for mode in ("O0", "O2"):
        ...
        reference = target / ("release" if mode == "O2" else "debug") / "cadkernel_acis_history_torus_reference.exe"
        ...
        mode_count = 0
        for label, radii, matrix_values, expected_failure in cases():
            values = [*(str(value) for value in radii), *(str(value) for value in matrix_values)]
