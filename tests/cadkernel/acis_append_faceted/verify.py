...
"""Compare native faceted append records with the pinned source and codec."""
...
def main() -> None:
    ...
    parent = ROOT / "build/cadkernel-acis-append-checks"
    ...
    (project / "Cargo.toml").write_text(
        '[package]\nname = "cadkernel_acis_append_reference"\nversion = "0.1.0"\nedition = "2021"\n'
    ...
    reference = target / "debug/cadkernel_acis_append_reference.exe"
    if not reference.is_file():
        reference = reference.with_suffix("")
    expected = normalize_output(checked([reference], cwd=ROOT))
    fixture = ROOT / "tests/required/cadkernel_acis_append_faceted"
    ...
    for mode in ("O0", "O2"):
        ...
        actual = normalize_output(checked([artifacts / f"cadkernel_acis_append_faceted-{mode}.out"], cwd=ROOT))
        compare(f"pinned source vs native {mode}", expected, actual)
        print(f"{mode}: {result}; {len(expected.splitlines())} record lines match", flush=True)
