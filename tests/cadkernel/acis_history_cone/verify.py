...
"""Compare the native Cone route with pinned public Rust history at O0/O2."""
...
def cases():
    for name, dimensions in (
        ("circular-point", (4., 4., 0., 7.)),
        ("elliptic-point", (4., 2., 0., 7.)),
        ("circular-frustum", (4., 4., 1.5, 7.)),
        ("elliptic-frustum", (4., 2., 1.5, 7.)),
    ):
    ...
    yield "near-circular-point", (4., 4. + 2e-13, 0., 7.), IDENTITY, 0
    yield "near-circular-frustum", (4., 4. + 2e-13, 1.5, 7.), IDENTITY, 0
    yield "near-point-circular", (4., 4., 1e-13, 7.), IDENTITY, 0
    yield "near-point-elliptic", (4., 2., 1e-13, 7.), IDENTITY, 0
    yield "negative-zero-top", (4., 2., -0., 7.), IDENTITY, 0
    yield "circular-cylinder-limit", (4., 4., 4., 7.), IDENTITY, 0
    yield "elliptic-cylinder-limit", (4., 2., 4., 7.), IDENTITY, 0
    yield "survey", (4., 2., 1.5, 7.), matrix(origin=(512345.678, 4512345.678, 91.5)), 0
    yield "affine-residue", (4., 2., 1.5, 7.), matrix(column=(5e-10, -5e-10, 5e-10, 1.)), 0
    for label, dimensions in (
        ("zero-base-x", (0., 2., 1.5, 7.)),
        ("negative-base-y", (4., -2., 1.5, 7.)),
        ("negative-top", (4., 2., -1., 7.)),
        ("nan-top", (4., 2., math.nan, 7.)),
        ("infinite-top", (4., 2., math.inf, 7.)),
        ("infinite-base-x", (math.inf, 2., 1.5, 7.)),
        ("zero-height", (4., 2., 1.5, 0.)),
        ("negative-height", (4., 2., 1.5, -7.)),
        ("nan-height", (4., 2., 1.5, math.nan)),
        ("infinite-height", (4., 2., 1.5, math.inf)),
    ):
    ...
    for label, placement in (
        ...
        yield label, (4., 2., 1.5, 7.), placement, 1
    yield "invalid-top-wins", (4., 2., -1., 7.), SHEAR, 2
    yield "invalid-base-wins", (0., 2., 1.5, 7.), matrix(column=(2e-9, 0., 0., 1.)), 2
    yield "invalid-height-wins", (4., 2., 1.5, 0.), NONUNIFORM, 2
    for index in range(8):
        dimensions = (3. + index / 8., 1.5 + index / 16., 1. + index / 32., 4. + index / 4.)
...
def main() -> None:
    ...
    parent = ROOT / "build/cadkernel-acis-history-cone-checks"
    ...
    (project / "Cargo.toml").write_text(
        '[package]\nname = "cadkernel_acis_history_cone_reference"\nversion = "0.1.0"\nedition = "2021"\n'
    ...
    fixture = ROOT / "tests/required/cadkernel_acis_history_cone"
    count = fields = 0
    for mode in ("O0", "O2"):
        ...
        reference = target / ("release" if mode == "O2" else "debug") / "cadkernel_acis_history_cone_reference.exe"
