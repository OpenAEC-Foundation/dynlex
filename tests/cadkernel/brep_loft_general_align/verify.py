...
"""Compare pinned Rust prepared() profile alignment with DynLex at O0/O2."""
...
def driver_text(source: Path) -> str:
    ...
    methods = "impl Bezier {\n" + "\n".join(source_function(text, signature, bezier) for signature in (
        "fn point(&self, t: f64)", "fn reversed(&self)", "fn split(&self, t: f64)", "fn part(&self, a: f64, b: f64)",
...
class Case:
    ...
    def arguments(self) -> list[str]:
        values = [int(self.cyclic), int(self.matching), len(self.sections)]
...
def compare(name: str, rust: str, native: str, expected_error: int) -> int:
    ...
    for _ in range(int(left[2])):
        if left[position + 12] != right[position + 12]:
            raise AssertionError(f"{name}: wire count differs")
        wire_count = int(left[position + 12])
        position += 13
...
def main() -> None:
    ...
    tested = fields = 0
    with tempfile.TemporaryDirectory(prefix="cad-loft-general-align-", dir=ROOT / "build") as directory:
        ...
        for mode, directory_name in (("O0", "debug"), ("O2", "release")):
            ...
            for item in cases():
                ...
                tested += 1
            if not args.case:
                fixture = ROOT / "tests/required/cadkernel_brep_loft_general_align"
                if fixture.is_dir():
