...
def closed_sheet_extent(name: str, fields: tuple[int, ...], points: list[tuple[float, float, float]]) -> tuple[float, ...]:
    ...
    if not points or any(not math.isfinite(value) for point in points for value in point):
        raise AssertionError(f"{name}: missing or nonfinite vertices")
    ...
    if sizes[0] <= 0 or sizes[1] <= 0:
        raise AssertionError(f"{name}: collapsed planar extent {sizes}")
    near_tuple(f"{name} extrusion height", (lows[2], highs[2]), (0.0, 3.0))
    return (*lows, *highs)
...
def compare(name: str, rust: str, native: str) -> None:
    ...
    near_tuple(f"{name} spatial extent", closed_sheet_extent(name, actual[2], actual[3]),
               closed_sheet_extent(name, expected[2], expected[3]))
...
def main() -> None:
    ...
    parser.add_argument("--reference-target", type=Path, default=ROOT / "build/cadkernel-thicken-angled-reference")
    ...
    if shutil.which("cargo") is None:
        raise AssertionError("cargo is required for the pinned reference build")
    reference_target = arguments.reference_target.resolve()
    project = reference_target / "project"
    (project / "src").mkdir(parents=True, exist_ok=True)
    (project / "src/main.rs").write_bytes((HERE / "reference.rs").read_bytes())
    (project / "Cargo.toml").write_text(
        '[package]\nname = "cadkernel_thicken_angled_reference"\nversion = "0.1.0"\nedition = "2021"\n'
        f'\n[dependencies]\ncadkernel = {{ path = "{source.as_posix()}", features = ["brep"] }}\n',
        encoding="utf-8",
    )
    suffix = ".exe" if sys.platform == "win32" else ".out"
    with tempfile.TemporaryDirectory(prefix="cad-thicken-multiface-angled-") as temporary:
        output = Path(temporary)
        modes = tuple(dict.fromkeys(arguments.mode or ("O0", "O2")))
        for mode in modes:
            directory = "debug" if mode == "O0" else "release"
            cargo = ["cargo", "build", "--offline", "--manifest-path", project / "Cargo.toml",
                     "--target-dir", reference_target / "target"]
            if mode == "O2":
                cargo.append("--release")
            run(cargo, timeout=600)
            rust_binary = reference_target / "target" / directory / f"cadkernel_thicken_angled_reference{suffix}"
            ...
            if arguments.reference_only:
                for case in range(CASES):
                    ...
                    if observed[0]:
                        closed_sheet_extent(f"case-{case}/{mode}", observed[2], observed[3])
                    print(f"RUST case-{case}/{mode}: valid={observed[0]} unchanged={observed[1]}", flush=True)
                continue
            diagnostics = run([arguments.compiler, HERE / "probe.dl", f"-{mode}", "-o", native_binary], timeout=420)
            if diagnostics:
