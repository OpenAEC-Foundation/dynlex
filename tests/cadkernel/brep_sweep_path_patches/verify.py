...
"""Compare isolated patch-building primitives with pinned Rust at O0/O2."""
...
CASES = (
    "line", "joined-lines", "opposite-turn", "planar-arc",
    "banked-arc", "reversed-arc", "planar-spline", "banked-spatial-spline",
    "zero-length-line", "unbanked-spatial-spline",
)
...
def numbers(label: str, output: str) -> list[float]:
    ...
    if len(values) < 23 or not all(map(math.isfinite, values)):
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="cad-path-patches-") as temporary:
        ...
        for mode, directory in (("O0", "debug"), ("O2", "release")):
            ...
            for case, name in enumerate(CASES):
                ...
                mode_values[name] = actual
                print(f"{name}/{mode}: {len(actual)} fields", flush=True)
            banked = mode_values["banked-spatial-spline"]
            unbanked = mode_values["unbanked-spatial-spline"]
            if len(banked) != len(unbanked) or not any(
                abs(left - right) > 1e-8 for left, right in zip(banked, unbanked)
            ):
                raise AssertionError(f"{mode}: curved banking did not alter the frame walk")
