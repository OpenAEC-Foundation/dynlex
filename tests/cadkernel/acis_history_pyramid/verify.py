...
"""Compare the native Pyramid route with pinned public Rust history at O0/O2."""
...
def cases():
    for sides in (3, 4, 6, 8):
        for top_radius in (0., 1.5):
            for placement_name, placement in (("identity", IDENTITY), ("turn", TURN),
                                              ("scale", SCALE), ("reflect", REFLECT)):
                name = "point" if top_radius == 0. else "frustum"
                yield f"{name}-{sides}-{placement_name}", (4., top_radius, 7., sides), placement, 0
    yield "near-point", (4., 1e-13, 7., 5), IDENTITY, 0
    yield "small-top", (4., 1e-9, 7., 5), IDENTITY, 0
    yield "negative-zero-top", (4., -0., 7., 5), IDENTITY, 0
    yield "equal-radii", (4., 4., 7., 5), IDENTITY, 0
    yield "survey", (4., 1.5, 7., 6), matrix(origin=(512345.678, 4512345.678, 91.5)), 0
    yield "affine-residue", (4., 1.5, 7., 6), matrix(column=(5e-10, -5e-10, 5e-10, 1.)), 0
    for sides in (-1, 0, 1, 2):
        yield f"sides-{sides}", (4., 1.5, 7., sides), IDENTITY, 2
    for label, dimensions in (
        ("zero-radius", (0., 1.5, 7., 4)),
        ("negative-radius", (-4., 1.5, 7., 4)),
        ("nan-radius", (math.nan, 1.5, 7., 4)),
        ("infinite-radius", (math.inf, 1.5, 7., 4)),
        ("negative-top", (4., -1., 7., 4)),
        ("nan-top", (4., math.nan, 7., 4)),
        ("infinite-top", (4., math.inf, 7., 4)),
        ("zero-height", (4., 1.5, 0., 4)),
        ("negative-height", (4., 1.5, -7., 4)),
        ("nan-height", (4., 1.5, math.nan, 4)),
        ("infinite-height", (4., 1.5, math.inf, 4)),
    ):
    ...
    for label, placement in (
        ...
        yield label, (4., 1.5, 7., 4), placement, 1
    yield "invalid-sides-wins", (4., 1.5, 7., 2), SHEAR, 2
    yield "invalid-top-wins", (4., -1., 7., 4), SHEAR, 2
    yield "invalid-height-wins", (4., 1.5, 0., 4), NONUNIFORM, 2
    for index in range(8):
        dimensions = (3. + index / 8., 1. + index / 16., 4. + index / 4., 3 + index)
        placement = matrix((0., 1., 0.), (-1., 0., 0.), (0., 0., 1.),
                           (index - 3., 2. * index, -index / 2.))
        yield f"varied-placed-{index}", dimensions, placement, 0
...
def main() -> None:
    ...
    parent = ROOT / "build/cadkernel-acis-history-pyramid-checks"
    ...
    (project / "Cargo.toml").write_text(
        '[package]\nname = "cadkernel_acis_history_pyramid_reference"\nversion = "0.1.0"\nedition = "2021"\n'
    ...
    fixture = ROOT / "tests/required/cadkernel_acis_history_pyramid"
    count = fields = 0
    for mode in ("O0", "O2"):
        ...
        reference = target / ("release" if mode == "O2" else "debug") / "cadkernel_acis_history_pyramid_reference.exe"
