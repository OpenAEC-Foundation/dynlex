...
"""Compare lower write ordering and refusal behavior with pinned kernel and codec."""
...
def main() -> None:
    ...
    parent = ROOT / "build/cadkernel-acis-lower-audit-checks"
    ...
    (project / "Cargo.toml").write_text(
        '[package]\nname = "cadkernel_acis_lower_audit_reference"\nversion = "0.1.0"\nedition = "2021"\n'
    ...
    reference = target / "debug/cadkernel_acis_lower_audit_reference.exe"
    if not reference.is_file():
        reference = reference.with_suffix("")
    expected = normalize_output(checked([reference], cwd=ROOT))
    fixture = ROOT / "tests/required/cadkernel_acis_lower_audit"
    ...
    for mode in ("O0", "O2"):
        ...
        actual = normalize_output(checked([artifacts / f"cadkernel_acis_lower_audit-{mode}.out"], cwd=ROOT))
