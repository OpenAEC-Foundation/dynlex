...
"""Compare native imprinting with the pinned Rust implementation."""
...
HERE = ROOT / "tests/cadkernel/brep_imprint"
...
def encoded(first_origin=(0.0, 0.0, 0.0), first_size=(10.0, 10.0, 10.0),
            second_origin=(5.0, 5.0, 5.0), second_size=(10.0, 10.0, 10.0),
            tolerance=1e-9, unknown=False):
    return [*first_origin, *first_size, *second_origin, *second_size,
            tolerance, float(unknown)]


def cases():
    yield "overlapping-corners", encoded()
    yield "disjoint", encoded(
        first_size=(1.0, 1.0, 1.0),
        second_origin=(50.0, 50.0, 50.0),
        second_size=(1.0, 1.0, 1.0),
    )
    yield "stacked-equal", encoded(second_origin=(0.0, 0.0, 10.0))
    yield "stacked-partial", encoded(
        second_origin=(0.0, 0.0, 10.0), second_size=(4.0, 4.0, 4.0),
    )
    yield "contained", encoded(
        second_origin=(2.0, 2.0, 2.0), second_size=(2.0, 2.0, 2.0),
    )
    yield "identical", encoded(second_origin=(0.0, 0.0, 0.0))
    yield "touching-face", encoded(second_origin=(10.0, 0.0, 0.0))
    yield "touching-edge", encoded(second_origin=(10.0, 10.0, 0.0))
    yield "touching-corner", encoded(second_origin=(10.0, 10.0, 10.0))
    yield "unknown", encoded(unknown=True)
    ...
    for index in range(80):
        ...
        first_size = tuple(scale * rng.uniform(0.75, 4.0) for _ in range(3))
        mode = index % 5
        if mode == 0:
            second_origin = tuple(
            ...
            second_origin = tuple(
                first_origin[axis] + first_size[axis] * rng.uniform(1.5, 3.0)
                for axis in range(3)
            )
        elif mode == 2:
            second_origin = (
                first_origin[0], first_origin[1], first_origin[2] + first_size[2],
            )
        elif mode == 3:
            second_origin = tuple(
                first_origin[axis] + first_size[axis] * rng.uniform(0.2, 0.4)
                for axis in range(3)
            )
        else:
            second_origin = first_origin
        second_size = tuple(scale * rng.uniform(0.5, 4.5) for _ in range(3))
        tolerance = max(1e-9, scale * 1e-8, world * 5e-15)
        yield f"random-{index}", encoded(
            first_origin, first_size, second_origin, second_size, tolerance,
        )
...
def main():
    ...
    output = args.output or Path(tempfile.mkdtemp(prefix="brep-imprint-", dir=ROOT / "build"))
    ...
    for name, values in cases():
        ...
        for mode in modes:
            rust_text, _ = process([binaries[mode]["rust"], *arguments], timeout=60)
            rust_values, limits = MEASURE["expected_values"](rust_text)
            native_text, _ = process([binaries[mode]["native"], *arguments], timeout=60)
    ...
    summary = {
        ...
        "native_source_sha256": {
            ...
            for path in (ROOT / "lib/cadkernel/brep_imprint.dl", HERE / "probe.dl", HERE / "main.dl")
        },
    }
