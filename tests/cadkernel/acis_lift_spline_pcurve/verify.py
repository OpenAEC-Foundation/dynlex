...
"""Compare a full-body spline-curve and pcurve SAT lift with pinned sources."""
...
def main() -> None:
    ...
    parent = ROOT / "build/cadkernel-acis-lift-spline-pcurve-checks"
    ...
    (project / "Cargo.toml").write_text(
        '[package]\nname = "cadkernel_acis_lift_spline_pcurve_reference"\nversion = "0.1.0"\nedition = "2021"\n'
    ...
    checked([*cargo, "build", "--offline", "--manifest-path", project / "Cargo.toml", "--target-dir", target], cwd=ROOT, timeout=420)
    reference = target / "debug/cadkernel_acis_lift_spline_pcurve_reference.exe"
    if not reference.is_file():
        reference = reference.with_suffix("")
    expected = normalize_output(checked([reference], cwd=ROOT))
    fixture = ROOT / "tests/required/cadkernel_acis_lift_spline_pcurve"
    if args.refresh_expected:
        (fixture / "expected.txt").write_text(expected + "\n", encoding="utf-8")
    compare("pinned source vs fixture oracle", normalize_output((fixture / "expected.txt").read_text(encoding="utf-8")), expected)

    if args.reference_only:
        print(f"Pinned Rust reference: {len(expected.splitlines())} lines", flush=True)
        return

    for mode in tuple(dict.fromkeys(args.mode or ("O0", "O2"))):
        result = verify_fixture(fixture, mode, compiler, artifacts, 300, 40, False)
        actual = normalize_output(checked([artifacts / f"cadkernel_acis_lift_spline_pcurve-{mode}.out"], cwd=ROOT))
