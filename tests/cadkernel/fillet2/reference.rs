...
fn fillet(f:geom2d::fillet::Fillet) {
    point(f.tangent1);point(f.tangent2);point(f.centre);
    number(f.start_angle);number(f.end_angle);integer(usize::from(f==f.clone()));
}
fn main() {
    use geom2d::fillet::*;
    ...
    let radius=a[1];
    if a[0]==0. {
        record_scale([a[3],a[4],radius]);
        let result=fillet_between_rays([a[3],a[4]],[a[5],a[6]],[a[7],a[8]],radius);
        integer(usize::from(result.is_some()));
        if let Some(f)=result{fillet(f);}
    } else {
        let size=a[9] as usize;
        let first=shape(&a[10..10+size]);
        let second=shape(&a[11+size..]);
        let original_first=first.clone();let original_second=second.clone();
        record_scale([radius]);
        let found=fillets_between(&first,&second,radius,Tolerance::new(a[2]));
        integer(found.len());
        for f in found{fillet(f);}
        integer(usize::from(first==original_first));integer(usize::from(second==original_second));
    }
}
