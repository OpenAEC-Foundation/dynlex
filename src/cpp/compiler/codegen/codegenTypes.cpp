...
convertSequentialAggregate(ParseContext &context, llvm::Value *value, const DataType &fromType, const DataType &toType) {
	requireCompilerInvariant(
		isSequentialAggregate(fromType) && isSequentialAggregate(toType) &&
	...
	requireCompilerInvariant(
		value->getType() == getLLVMType(context, fromType),
		"sequential aggregate conversion value has the wrong source ABI"
	);
	...
	for (int elementIndex = 0; elementIndex < fromType.arraySize; elementIndex++) {
		...
		result =
			toType.kind == DataType::Kind::Array
