...
inline bool flexPatternCanForwardControlHeader(PatternDefinition *definition, bool outermost) {
	if (!definition || !definition->section || !definition->section->isFlex)
		return false;
	if (outermost)
		return definition->section->type == SectionType::Section;
	bool hasSingleBodyExpression = false;
	flexPatternBodyExpression(definition, &hasSingleBodyExpression);
	return definition->section->type == SectionType::Section || hasSingleBodyExpression;
}
