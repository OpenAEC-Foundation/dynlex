...
bool ListingSection::processLine(ParseContext &context, CodeLine *line) {
	if (!parseCommaSeparatedListWithRanges(line->patternText, [&](std::string_view /*item*/, size_t start, size_t end) {
		return addItem(context, Range(line, line->patternText.substr(start, end - start)));
